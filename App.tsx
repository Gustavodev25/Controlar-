...
const App: React.FC = () => {
  ...
  // Gerar lembretes automáticos das faturas dos cartões de crédito conectados
  const creditCardReminders = useMemo((): Reminder[] => {
    const cardReminders: Reminder[] = [];

    // Filtrar apenas cartões de crédito
    const creditCards = connectedAccounts.filter(acc =>
      acc.isCredit ||
      (acc.type?.toUpperCase().includes('CREDIT')) ||
      (acc.subtype?.toUpperCase().includes('CREDIT'))
    );

    creditCards.forEach(card => {
      // Tentar pegar a data de vencimento da fatura atual
      // Prioridade: currentBill.dueDate > invoicePeriods.currentInvoice.dueDate > balanceDueDate
      let currentDueDate = card.currentBill?.dueDate ||
        card.invoicePeriods?.currentInvoice?.dueDate ||
        card.balanceDueDate;

      // Normalizar data (remover parte T se existir)
      if (currentDueDate && currentDueDate.includes('T')) {
        currentDueDate = currentDueDate.split('T')[0];
      }

      // Tentar pegar o valor da fatura atual
      const currentBillAmount = card.currentBill?.totalAmount ??
        card.usedCreditLimit ??
        0;

      // Se tem data de vencimento, criar o lembrete da fatura atual
      if (currentDueDate) {
        const invoiceMonth = currentDueDate.substring(0, 7); // YYYY-MM

        cardReminders.push({
          id: `card_invoice_${card.id}_${invoiceMonth}`,
          description: `Fatura ${card.name || card.connector?.name || 'Cartão'}`,
          ...
          autoGenerated: true,
          cardId: card.id,
          cardName: card.name || card.connector?.name || 'Cartão',
          invoiceMonth: invoiceMonth
        });
      }

      // Também verificar a próxima fatura (para visão futura)
      let nextDueDate = card.invoicePeriods?.nextInvoice?.dueDate;
      if (nextDueDate && nextDueDate.includes('T')) {
        nextDueDate = nextDueDate.split('T')[0];
      }

      if (nextDueDate && nextDueDate !== currentDueDate) {
        const nextInvoiceMonth = nextDueDate.substring(0, 7);

        cardReminders.push({
          id: `card_invoice_${card.id}_${nextInvoiceMonth}`,
          description: `Fatura ${card.name || card.connector?.name || 'Cartão'}`,
          amount: 0, // Próxima fatura ainda não tem valor definido
          dueDate: nextDueDate,
          category: 'Cartão de Crédito',
          type: 'expense',
          isRecurring: true,
  ...
  // Combinar lembretes manuais + lembretes automáticos de cartão
  const allReminders = useMemo(() => {
    // Evitar duplicatas: se já existe um lembrete manual com o mesmo cardId e invoiceMonth, não adicionar
    const existingCardKeys = new Set(
      reminders
        .filter(r => r.cardId && r.invoiceMonth)
        .map(r => `${r.cardId}_${r.invoiceMonth}`)
    );

    const filteredCardReminders = creditCardReminders.filter(
      cr => !existingCardKeys.has(`${cr.cardId}_${cr.invoiceMonth}`)
    );

    return [...reminders, ...filteredCardReminders];
  }, [reminders, creditCardReminders]);

  // 3. Filter Reminders (agora usa allReminders que inclui lembretes automáticos de cartão)
  const filteredReminders = useMemo(() => {
    if (activeMemberId === 'FAMILY_OVERVIEW') return allReminders;
    return allReminders.filter(t => !t.memberId || t.memberId === activeMemberId);
  }, [allReminders, activeMemberId]);
