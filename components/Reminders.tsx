...
import { Reminder, ConnectedAccount } from '../types';
import { CalendarClock, Check, Trash2, AlertCircle, DollarSign, Tag, Calendar, getCategoryIcon, X, LayoutDashboard, Table2, FileText, Sparkles, Plus, Bot, ArrowRight, TrendingUp, TrendingDown, RefreshCw, AlertTriangle, Edit2, Send, User, Clock, ChevronLeft, ChevronRight, CreditCard } from './Icons';
      ...
      <div className="flex items-center gap-2 sm:gap-4 px-3 sm:px-5 py-3 sm:py-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5">
            ...
            {item.autoGenerated && (
              <span className="flex items-center gap-1 text-[10px] text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded-md border border-blue-500/30">
                <CreditCard size={9} />
                <span className="hidden sm:inline">Auto</span>
              </span>
            )}
            {item.isRecurring && !item.autoGenerated && (
        ...
        <div className={`flex items-center gap-0.5 sm:gap-1 transition-all duration-300 ${selectionMode ? 'opacity-0 pointer-events-none w-0' : 'sm:opacity-0 sm:group-hover:opacity-100 sm:-mr-2 sm:group-hover:mr-0'}`}>
          ...
          {/* Editar e Excluir - apenas para lembretes manuais */}
          {!item.autoGenerated && (
            <>
              <button
          ...
          {/* Para lembretes automáticos, mostra tooltip informativo */}
          {item.autoGenerated && (
            <Tooltip content="Gerado automaticamente da fatura do cartão">
              <span className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg text-blue-400">
                <CreditCard size={14} />
              </span>
            </Tooltip>
          )}
...
export const Reminders: React.FC<RemindersProps> = ({ reminders, onAddReminder, onDeleteReminder, onPayReminder, onUpdateReminder, onOpenAIModal, isProMode = false, userPlan = 'starter', onUpgrade, userId, connectedAccounts = [] }) => {
