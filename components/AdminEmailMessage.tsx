...
import { getAllUsers, addNotification, addPromoPopup, getWaitlistEntries, saveEmailDraft, getEmailDraft, addAdminMessageLog, listenToAdminMessageLogs, AdminMessageLog } from '../services/database';
            ...
            if (deliveryMethod === 'notification') {
                ...
                if (successCount > 0) {
                    await addAdminMessageLog({
                        ...
                        sentBy: currentUser?.email || 'admin',
                        status: errorCount === 0 ? 'success' : 'partial',
                        details: { popupType, hasImage: !!finalImageUrl }
                    });
                }
            } else {
                // Send Email
                ...
                if (response.ok) {
                    ...
                    await addAdminMessageLog({
                        ...
                        successCount: recipients.length, // Providing API success assumed all good
                        failureCount: 0,
                        sentAt: new Date().toISOString(),
                        sentBy: currentUser?.email || 'admin',
                        status: 'success'
        ...
        <div className="flex flex-col h-full animate-fade-in">
            ...
            <div className="mb-6 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    <div className="flex bg-gray-900 p-1 rounded-xl border border-gray-800 mr-4">
                        <button
                            onClick={() => setActiveTab('compose')}
                            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'compose' ? 'bg-gray-800 text-white shadow-md' : 'text-gray-500 hover:text-gray-400'}`}
                        >
                            Nova Mensagem
                        </button>
                        <button
                            onClick={() => setActiveTab('history')}
                            className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'history' ? 'bg-gray-800 text-white shadow-md' : 'text-gray-500 hover:text-gray-400'}`}
                        >
                            Histórico
                        </button>
                    </div>
...
function HistoryView() {
    ...
    useEffect(() => {
        const unsubscribe = listenToAdminMessageLogs((fetchedLogs) => {
            setLogs(fetchedLogs);
            setLoading(false);
        });
        return unsubscribe;
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            ...
            case 'notification': return <Bell size={16} className="text-[#d97757]" />;
            case 'popup': return <Sparkles size={16} className="text-purple-400" />;
    ...
    const getRecipientLabel = (type: string) => {
        switch (type) {
            ...
            case 'pro': return 'Premium (Pro/Family)';
            case 'starter': return 'Starter';
            case 'waitlist': return 'Lista de Espera';
            case 'specific': return 'Usuários Específicos';
            default: return type;
        }
    };
    ...
    if (loading) {
        return (
            <div className="flex-1 flex items-center justify-center h-[500px]">
                <div className="w-8 h-8 border-4 border-[#d97757]/30 border-t-[#d97757] rounded-full animate-spin"></div>
            </div>
        );
    }
    ...
    if (logs.length === 0) {
        return (
            <EmptyState
                ...
                description="Você ainda não enviou mensagens. Envie notificações, popups ou emails para ver o registro aqui."
                minHeight="h-full"
                className="flex-1"
            />
        );
    ...
    return (
        <div className="flex-1 bg-gray-950 border border-gray-800 rounded-2xl shadow-xl overflow-hidden flex flex-col min-h-0">
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-900 sticky top-0 z-10">
                        <tr>
                            ...
                            <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Título/Assunto</th>
                            <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Público</th>
                            <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Enviados</th>
                            <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-800">
                        {logs.map((log) => (
                            <tr key={log.id} className="hover:bg-gray-900/30 transition-colors group">
                                <td className="p-4 text-xs text-gray-400 whitespace-nowrap">
                                    <div className="flex items-center gap-2">
                                        <Calendar size={12} />
                                        {new Date(log.sentAt).toLocaleDateString()}
                                        <span className="opacity-50">{new Date(log.sentAt).toLocaleTimeString().slice(0, 5)}</span>
                                    </div>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4">
                                    <div className="max-w-[250px]">
                                        <div className="text-sm font-bold text-white truncate" title={log.subject || log.title}>
                                            {log.subject || log.title}
                                        </div>
                                        <div className="text-xs text-gray-500 truncate" title={log.body}>
                                            {log.body}
                                        </div>
                                    </div>
                                </td>
                                <td className="p-4">
                                    <div className="text-xs text-gray-300 bg-gray-800/50 px-2 py-1 rounded inline-block border border-gray-700/50">
                                        {getRecipientLabel(log.recipientType)}
                                    </div>
                                </td>
                                <td className="p-4 text-right">
                                    <div className="text-sm font-bold text-white">{log.successCount || log.recipientCount}</div>
                                    {log.failureCount ? (
                                        <div className="text-[10px] text-red-400">{log.failureCount} falhas</div>
                                    ) : null}
                                </td>
                                <td className="p-4">
                                    <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${log.status === 'success'
                                        ? 'bg-green-500/10 text-green-400 border border-green-500/20'
                                        : log.status === 'partial'
                                            ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'
                                            : 'bg-red-500/10 text-red-400 border border-red-500/20'
                                        }`}>
                                        <div className={`w-1.5 h-1.5 rounded-full ${log.status === 'success' ? 'bg-green-500' : log.status === 'partial' ? 'bg-yellow-500' : 'bg-red-500'
                                            }`} />
                                        {log.status === 'success' ? 'Enviado' : log.status === 'partial' ? 'Parcial' : 'Erro'}
                                    </div>
                                </td>
                            </tr>
                        ))}
