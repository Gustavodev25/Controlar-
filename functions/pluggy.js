...
const getPluggyApiKey = async () => {
    ...
    if (cachedApiKey && Date.now() < cachedApiKeyExpiry) {
        console.log('>>> Using in-memory cached Pluggy API Key (valid for', Math.round((cachedApiKeyExpiry - Date.now()) / 1000), 'seconds)');
        return cachedApiKey;
    }
    ...
    if (firebaseAdmin) {
        try {
            ...
            const tokenDoc = await db.collection('system_config').doc('pluggy_token').get();
            ...
            if (tokenDoc.exists) {
                ...
                if (expiresAt && Date.now() < expiresAt) {
                    // Token is still valid - update in-memory cache and return
                    cachedApiKey = tokenData.apiKey;
                    cachedApiKeyExpiry = expiresAt;
                    console.log('>>> Using Firestore cached Pluggy API Key (valid for', Math.round((expiresAt - Date.now()) / 1000), 'seconds)');
                    return cachedApiKey;
                }
            }
        } catch (e) {
            console.warn('>>> Could not read Pluggy token from Firestore cache:', e.message);
    ...
    authInProgress = (async () => {
        try {
            ...
            if (firebaseAdmin) {
                try {
                    const db = firebaseAdmin.firestore();
                    await db.collection('system_config').doc('pluggy_token').set({
                        apiKey: cachedApiKey,
                        expiresAt: firebaseAdmin.firestore.Timestamp.fromMillis(cachedApiKeyExpiry),
                        createdAt: firebaseAdmin.firestore.FieldValue.serverTimestamp(),
                        generatedBy: IS_VERCEL ? 'vercel' : 'local'
                    });
                    console.log('>>> Pluggy Token saved to Firestore cache');
                } catch (e) {
                    console.warn('>>> Could not save Pluggy token to Firestore cache:', e.message);
                }
            }
...
const enqueueSyncQueueJob = async ({ type, userId, itemId, syncJobId, creditTransactionId }) => {
    if (!firebaseAdmin) throw new Error('Firebase Admin not initialized');
    const db = firebaseAdmin.firestore();
    const now = Date.now();

    const docId = syncJobId ? `${syncJobId}_${type || 'sync'}` : undefined;
    const jobRef = docId
        ? db.collection(SYNC_QUEUE_COLLECTION).doc(docId)
        : db.collection(SYNC_QUEUE_COLLECTION).doc();
    ...
    try {
        await jobRef.create(jobData);
        return { duplicate: false, jobId: jobRef.id };
    } catch (e) {
        const code = e?.code;
        const msg = String(e?.message || '');
        const alreadyExists = code === 6 || code === 'already-exists' || msg.includes('Already exists');
        if (alreadyExists) return { duplicate: true, jobId: jobRef.id };
        throw e;
    }
};
...
router.post('/trigger-sync', async (req, res) => {
    ...
    try {
        ...
        // 3. Queue background processing in Firebase Functions
        await updateSyncStatus(userId, 'pending', 'Solicitando atualização ao banco...');
        const queued = await enqueueSyncQueueJob({
            type: 'trigger',
            userId,
            itemId,
            syncJobId,
        ...
        res.json({
            success: true,
            message: 'Processo de sincronização enfileirado.',
            syncJobId,
            queued
        });
        ...
        if (creditTransactionId) {
            await refundCredit(userId, creditTransactionId, 'QUEUE_ERROR');
        }
...
router.post('/sync', async (req, res) => {
    ...
    try {
        ...
        // 4. Immediate Response to Frontend with syncJobId for tracking
        res.json({
            success: true,
...
export { processSyncWithRefund, pollAndSyncWithRefund, getPluggyApiKey, pluggyRequest };
