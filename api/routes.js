  ...
  try {
    ...
    if (billingCycle === 'annual' && installmentCount && installmentCount > 1) {
      const paymentData = {
        ...
        value: recurringValue, // Annual plan with installments always uses full value partitioned
        dueDate: dueDateStr,
        description: `Plano ${planId} - Anual (${installmentCount}x)`,
        installmentCount: installmentCount,
        installmentValue: Math.round((recurringValue / installmentCount) * 100) / 100,
        creditCard: {
      ...
      // Handle First Payment Discount Logic
      // If the effective first payment (value) is less than the recurring price (recurringValue),
      // we need to update the first generated payment to reflect the discount.
      // This ensures the subscription continues at full price (recurringValue) from month 2 onwards.
      if (value < recurringValue) {
        console.log(`>>> Applying first month discount: ${recurringValue} -> ${value}`);
        try {
          // Fetch the first payment generated by this subscription
          const payments = await asaasRequest('GET', `/payments?subscription=${subscription.id}&limit=1`);
          const firstPaymentId = payments.data?.[0]?.id;

          if (firstPaymentId) {
            // Update the first payment value
            // We can also add a description or discount field if needed, but updating value is direct.
            await asaasRequest('POST', `/payments/${firstPaymentId}`, {
              value: value
            });
            console.log(`>>> Updated first payment ${firstPaymentId} to ${value}`);
          }
        } catch (updateError) {
          console.error('>>> Error updating first payment discount:', updateError.message);
          // Continue execution, as the subscription was created.
          // In a production env, might want to alert admin or retry.
        }
      }
